...
> [!IMPORTANT]
> Все системные ScriptableObject (Variable, Event, Settings) **ОБЯЗАНЫ** находиться в папке `Assets/Game/Architecture/` и создаваться через контекстное меню в разделе **Architecture**.
...
#### **IInteractable** - Базовый интерфейс взаимодействия
**Файл:** `Assets/Game/Scripts/Inteface/IInteractable.cs`
Используется для быстрых взаимодействий на клавишу **E** (Кровати, мониторы).
```csharp
...
**Назначение:** Обработка всех действий игрока (строительство, взаимодействие с объектами) и динамическая подсветка интерактивных объектов.

**Логика подсветки (Highlight System):**
- **Highlight (Контур)**: При наведении взгляда на `IInteractable` или `IInteractableTrap`, скрипт находит все `Renderer` (включая дочерние) и временно добавляет `highlightMaterial` в их массив `sharedMaterials`.
- **Hierarchical Support**: Подсвечивается весь объект полностью (родители и дети), даже если коллайдер находится на отдельном подобъекте.
- **Очистка**: Подсветка снимается автоматически при смене цели, входе в режим строительства или поднятии предмета.
- **Безопасность**: Метод `ClearHighlight()` удаляет только материал подсветки, не затрагивая другие эффекты (например, Ghost Mode монстра).
...
- **Установка:** ЛКМ с удержанием (`placeHoldTimeRequired`) устанавливает предмет. 
  - Камеры (2): ставятся только на `treeLayer`, используют настройки `cameraBuildDistance` и `cameraLookAtPlayer` (поворот к игроку).
  - Ловушки (1): ставятся только на `groundLayer` в пределах `buildDistance`.
...
1. **Поднятие:** Удержание E на объекте → `ProcessHold()` → `PerformPickup()`
   - Если ловушка пустая → возврат в инвентарь (`trapsCount++`) и полное удаление объекта (ищет самый верхний корень с компонентом ловушки).
...
### 4.1 **TrapBox.cs** - Модульная ловушка (Клетка)

**Назначение:** Захват существ и их транспортировка.

**Особенности:**
- **Позиционирование**: Использует `captureCenterPoint` и переменную `catchVisualOffset` для точной настройки положения монстра внутри клетки (исправляет "парение" моделей разного роста).
- **Логика**: При поимке отключает AI монстра, притягивает к центру через DOTween и включает физику клетки.
- **Доставка**: При касании `ParkTrigger` отправляет данные монстра в `ParkManager` и уничтожается.
...
3. **Просмотр карты** (`isWatchingMap`):
   ...
   - Рассылает сигналы `onMapEntered` / `onMapExited` для включения спец-света
   - Возврат в меню: E/Escape
...
**Назначение:** Создание врагов ночью в фиксированных точках с сохранением их типа.

**Логика спавна:**

1. **SpawnAssignments (Назначения):** Система сохраняет связь между точкой спавна и конкретным префабом врага (класс `SpawnAssignment`).
   - Если враг был пойман (`IsCaught == true`), назначение удаляется. На следующую ночь для этого "слота" будет выбрана новая рандомная точка и тип врага.
   - Если враг НЕ пойман, он удаляется утром, но его назначение остается. Следующей ночью на той же точке появится тот же тип врага.

2. **Массив видов:** Поддерживает массив `enemyPrefabs`, позволяя спавнить разных врагов.

3. **Спавн по окружности:** Враги спавнятся в радиусе `spawnRadius` от назначенной точки.

**Методы:**
- `SpawnEnemies()` - добирает недостающие назначения до `enemiesPerNight` и создает монстров.
- `ClearEnemies()` - вызывается утром. Удаляет объекты свободных монстров, сбрасывает назначения для пойманных.
...
**Назначение:** Управление ортографической камерой и освещением в режиме карты.

**Наследование:** `SignalBinder`

**Ключевые компоненты:**

- **Управление:** `panSpeed`, `zoomSpeed`, `minZoom`, `maxZoom` и границы перемещения.
- **Освещение (Map Visuals):**
  - `mapVisualLight` — выделенный источник света, который включается только при просмотре карты.
  - Слушает события `GET_onMapEntered` и `GET_onMapExited` для переключения этого света.

**Интеграция:**
- Наследует `SignalBinder` для автоматической подписки на сигналы входа/выхода на карту.
- Работает в связке с `CctvManager` и `MapUIHandler`.
...
### 6. **Рендеринг карты:**
   - Использует выделенный свет `mapVisualLight`.
   - Включается автоматически через `GameEvent` при переходе в режим карты в `CctvManager`.
...
> [!CAUTION]
> **ПРАВИЛО ПАПОК:** Все системные ассеты (Variables, Events, Settings) должны лежать в `Assets/Game/Architecture/`. Создавай их через меню **Create -> Architecture**. Папку с названием "Architecture" в папке Scripts создавать нельзя.

1. **Hierarchy Awareness**: При подсветке или эффектах всегда ищи компоненты `Renderer` во всех вложенных объектах (`GetComponentsInChildren`).
2. **Material Management**: Используй `sharedMaterials` для динамических эффектов. При удалении контура удаляй только последний элемент массива, чтобы не сломать другие эффекты.
3. **Trap Positioning**: Для настройки положения существа в клетке всегда используй `catchVisualOffset` в `TrapBox`.
4. **Interface Priority**: Для кликов по E — `IInteractable`. Для переноски — `IInteractableTrap`. Для эффектов монстра — `IGhostable`.
5. **SignalBinder**: Всегда вызывай `base.OnEnable()` и `base.OnDisable()` при наследовании от `SignalBinder`.

---

**Версия документа:** 5.1 (Actualized & Merged)

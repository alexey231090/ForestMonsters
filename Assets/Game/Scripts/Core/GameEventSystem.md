...
## 🏛 ПРАВИЛО №0: СОЗДАНИЕ АССЕТОВ
Все системные ScriptableObject (Variable, Event, Settings) **ОБЯЗАНЫ** создаваться через контекстное меню в разделе **Architecture**.
- Это гарантирует правильную структуру проекта и единообразие.
- Пример: `Create -> Architecture -> Variables -> Int`.
- Пример: `Create -> Architecture -> Settings -> Ghost Effects`.
...
## ⚖️ ВЫБОР: INTERFACE VS SO-SIGNALS

Для поддержания чистоты и производительности следуй этим правилам:

### 1. Когда использовать INTERFACE (`IGhostable`, `IInteractable`)
*Лучше для: Прямого, точечного взаимодействия между двумя конкретными объектами.*
- **Ситуация**: Объект А точно знает, на каком объекте Б он хочет вызвать действие (например, ловушка несет конкретного монстра).
- **Плюсы**: 
    - Максимальная производительность (прямой вызов метода).
    - Точность (эффект только на одном объекте, а не на всех монстрах в мире).
    - Читаемость: в коде четко видно `ghostable.SetGhostMode(true)`.

### 2. Когда использовать SO-SIGNALS (`GameEvent`, `[Bind]`)
*Лучше для: Глобальных событий, когда "один кричит — многие слушают".*
- **Ситуация**: Произошло событие, о котором должны узнать многие независимые системы (например, `EV_OnPlayerDeath` — обновляет UI, респавнит врагов, проигрывает звук).
- **Плюсы**: 
    - Полная развязка модулей (объект не знает, кто его слушает).
    - Удобство настройки в инспекторе без написания кода.
    - Легко расширять (добавил новый скрипт-слушатель — и всё работает).

### 📝 Резюме по читаемости:
- Если нужно **"ткнуть пальцем"** в конкретный объект — используй **Interface**.
- Если нужно **"объявить на весь мир"** — используй **SO-Signals**.
- Если нужны **глобальные настройки** для многих объектов (цвета, материалы) — используй **SO-Settings** (часто в комбинации с интерфейсом или сигналами).
